        ...
        return fromCol.find({}).fetch((items) => {
            ...
            return toCol.seed(items, () => {
                ...
                return fromCol.pendingUpserts((upserts) => {
                    return toCol.upsert(lodash_1.default.pluck(upserts, "doc"), lodash_1.default.pluck(upserts, "base"), () => {
                        // Copy removes
...
function processFind(items, selector, options) {
    ...
    if (options && options.sort) {
        filtered.sort((0, selector_2.compileSort)(options.sort));
    }
...
function processNearOperator(selector, list) {
    for (var key in selector) {
        ...
        if (value != null && value["$near"]) {
            ...
            // Extract docs
            list = lodash_1.default.pluck(distances, "doc");
        }
    }
    return list;
}
