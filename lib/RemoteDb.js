...
class Collection {
    ...
    upsert(docs, bases, success, error) {
        ...
        const basesPresent = lodash_1.default.compact(lodash_1.default.pluck(items, "base")).length > 0;
        const params = {};
        if (this.client) {
            params.client = this.client;
        }
        ...
        if (items.length === 1) {
            ...
            // POST if no base, PATCH otherwise
            if (basesPresent) {
                return this.httpClient("PATCH", this.getUrl(), params, { doc: lodash_1.default.pluck(items, "doc"), base: lodash_1.default.pluck(items, "base") }, (result) => success(result), function (err) {
                    if (error) {
                        return error(err);
                    }
                });
                ...
                return this.httpClient("POST", this.getUrl(), params, lodash_1.default.pluck(items, "doc"), (result) => success(result), function (err) {
                    if (error) {
                        return error(err);
                    }
                });
