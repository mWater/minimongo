...
doc TEXT,
base TEXT,
PRIMARY KEY (col, id));`, [], doNothing, (tx, err) => {
...
ALTER TABLE docs ADD COLUMN base TEXT;`, [], doNothing, (tx, err) => error(err));
    ...
    removeCollection(name, success, error) {
        ...
        this.db.transaction((tx) => tx.executeSql("DELETE FROM docs WHERE col = ?", [name], success, (tx, err) => error(err)), error);
    }
    getCollectionNames() {
...
class Collection {
    ...
    _findFetch(selector, options, success, error) {
        ...
        this.db.readTransaction((tx) => {
            // 1) get total count
            tx.executeSql("SELECT COUNT(*) AS count FROM docs WHERE col = ? AND state <> ?", [this.name, "removed"], (tx, r) => {
                const total = r.rows.item(0).count;
                // 2) recursive batch fetch — *all* inside the same tx
                const fetchBatch = () => {
                    tx.executeSql("SELECT * FROM docs WHERE col = ? AND state <> ? ORDER BY id LIMIT ? OFFSET ?", [this.name, "removed", BATCH_SIZE, offset], (tx, rs) => {
                        ...
                        if (offset < total) {
                            ...
                            // 3) all done
                            try {
                                success((0, utils_1.processFind)(docs, selector, options));
                            }
                            catch (e) {
                                error(e);
                            }
    ...
    uncache(selector, success, error) {
        ...
        return this.db.transaction((tx) => {
            ...
            tx.executeSql("SELECT COUNT(*) AS count FROM docs WHERE col = ? AND state = ?", [this.name, "cached"], (tx, rs) => {
                const total = rs.rows.item(0).count;
                // Recursive batch fetch
                const fetchBatch = () => {
                    tx.executeSql("SELECT * FROM docs WHERE col = ? AND state = ? ORDER BY id LIMIT ? OFFSET ?", [this.name, "cached", BATCH_SIZE, offset], (tx, rs2) => {
                        ...
                        if (offset < total) {
                            ...
                            // Perform deletions
                            async_1.default.eachSeries(toRemove, ((id, callback) => {
