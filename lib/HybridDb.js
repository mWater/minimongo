...
class HybridDb {
    ...
    upload(success, error) {
        const cols = lodash_1.default.values(this.collections);
        function uploadCols(cols, success, error) {
            const col = lodash_1.default.first(cols);
            if (col) {
                return col.upload(() => uploadCols(lodash_1.default.rest(cols), success, error), (err) => error(err));
            }
            else {
                return success();
            }
...
class HybridCollection {
                ...
                const step2 = (localData) => {
                    ...
                    const remoteSuccess = (remoteData) => {
                        ...
                        if (options.cacheFind) {
                            ...
                            let data = remoteData;
                            if (removes.length > 0) {
                                const removesMap = lodash_1.default.object(lodash_1.default.map(removes, (id) => [id, id]));
                                data = lodash_1.default.filter(remoteData, (doc) => !lodash_1.default.has(removesMap, doc._id));
                            }
                            // Add upserts
                            if (upserts.length > 0) {
                                // Remove upserts from data
                                const upsertsMap = lodash_1.default.object(lodash_1.default.map(upserts, (u) => u.doc._id), lodash_1.default.map(upserts, (u) => u.doc._id));
                                data = lodash_1.default.filter(data, (doc) => !lodash_1.default.has(upsertsMap, doc._id));
                                // Add upserts
                                data = data.concat(lodash_1.default.pluck(upserts, "doc"));
                                // Refilter/sort/limit
    ...
    upsert(docs, bases, success, error) {
        ...
        return this.localCol.upsert(lodash_1.default.pluck(items, "doc"), lodash_1.default.pluck(items, "base"), (result) => success === null || success === void 0 ? void 0 : success(docs), error);
    }
            ...
            if (upsert) {
                return this.remoteCol.upsert(upsert.doc, upsert.base, (remoteDoc) => {
                    return this.localCol.resolveUpserts([upsert], () => {
                        ...
                        if (remoteDoc) {
                            return this.localCol.cacheOne(remoteDoc, () => uploadUpserts(lodash_1.default.rest(upserts), success, error), error);
                        }
                        else {
                            // Remove local
                            return this.localCol.remove(upsert.doc._id, () => {
                                // Resolve remove
                                return this.localCol.resolveRemove(upsert.doc._id, () => uploadUpserts(lodash_1.default.rest(upserts), success, error), error);
                            }, error);
                        }
                    }, error);
                }, (err) => {
                    ...
                    if (err.status === 410 || err.status === 403) {
                        return this.localCol.remove(upsert.doc._id, () => {
                            ...
                            return this.localCol.resolveRemove(upsert.doc._id, function () {
                                // Continue if was 410
                                if (err.status === 410) {
                                    return uploadUpserts(lodash_1.default.rest(upserts), success, error);
                                }
                                else {
                                    return error(err);
                                }
        ...
        var uploadRemoves = (removes, success, error) => {
            ...
            if (remove) {
                return this.remoteCol.remove(remove, () => {
                    return this.localCol.resolveRemove(remove, () => uploadRemoves(lodash_1.default.rest(removes), success, error), error);
                }, (err) => {
                    ...
                    if (err.status === 410 || err.status === 403) {
                        return this.localCol.resolveRemove(remove, function () {
                            // Continue if was 410
                            if (err.status === 410) {
                                return uploadRemoves(lodash_1.default.rest(removes), success, error);
                            }
                            else {
                                return error(err);
                            }
