...
export default WebSQLDb = class WebSQLDb {
  constructor(options, success, error) {
    ...
    const createTables = tx => tx.executeSql(`\
...
PRIMARY KEY (col, id));`, [], doNothing, error);
    ...
    this.db.transaction(createTables, error, () => {
      if (success) { return success(this); }
     });
  }
  ...
  removeCollection(name, success, error) {
    ...
    return this.db.transaction(tx => tx.executeSql("DELETE FROM docs WHERE col = ?", [name], success, error)
    , error);
  }
};
...
class Collection {
  ...
  _findFetch(selector, options, success, error) {
    ...
    return this.db.readTransaction(tx => {
      return tx.executeSql("SELECT * FROM docs WHERE col = ?", [this.name], function(tx, results) {
        ...
        if (success != null) { return success(processFind(docs, selector, options)); }
      }
      , error);
    }
    , error);
  ...
  upsert(doc, success, error) {
    ...
    return this.db.transaction(tx => {
      return (() => {
        const result = [];
        for (item of items) {
          result.push(tx.executeSql("INSERT OR REPLACE INTO docs (col, id, state, doc) VALUES (?, ?, ?, ?)", [this.name, item._id, "upserted", JSON.stringify(item)], doNothing, error));
        }
        return result;
      })();
    }
  ...
  cache(docs, selector, options, success, error) {
    ...
    return this.db.transaction(tx => {
      ...
      return async.eachSeries(docs, (doc, callback) => {
        return tx.executeSql("SELECT * FROM docs WHERE col = ? AND id = ?", [this.name, doc._id], (tx, results) => {
          ...
          if ((results.rows.length === 0) || (results.rows.item(0).state === "cached")) {
            ...
            if (!existing || !doc._rev || !existing._rev || (doc._rev >= existing._rev)) {
              // Upsert
              return tx.executeSql("INSERT OR REPLACE INTO docs (col, id, state, doc) VALUES (?, ?, ?, ?)", [this.name, doc._id, "cached", JSON.stringify(doc)], () => callback()
              , error);
            } else {
              return callback();
            }
      ...
      , err => {
        ...
        return this.find(selector, options).fetch(results => {
          return this.db.transaction(tx => {
            return async.eachSeries(results, (result, callback) => {
              ...
              return tx.executeSql("SELECT * FROM docs WHERE col = ? AND id = ?", [this.name, result._id], (tx, rows) => {
                if (!docsMap[result._id] && (rows.rows.length > 0) && (rows.rows.item(0).state === "cached")) {
                  ...
                  return tx.executeSql("DELETE FROM docs WHERE col = ? AND id = ?", [this.name, result._id], () => callback()
                  , error);
                } else {
                  return callback();
                }
  ...
  pendingUpserts(success, error) {
    ...
    return this.db.readTransaction(tx => {
      return tx.executeSql("SELECT * FROM docs WHERE col = ? AND state = ?", [this.name, "upserted"], function(tx, results) {
        ...
        if (success != null) { return success(docs); }
      }
      , error);
    }
    , error);
  ...
  resolveUpsert(doc, success, error) {
    ...
    // Find records
    return this.db.transaction(tx => {
      return async.eachSeries(items, (item, cb) => {
        return tx.executeSql("SELECT * FROM docs WHERE col = ? AND id = ?", [this.name, item._id], (tx, results) => {
          if (results.rows.length > 0) {
            ...
            if ((results.rows.item(0).state === "upserted") && _.isEqual(JSON.parse(results.rows.item(0).doc), item)) {
              tx.executeSql('UPDATE docs SET state="cached" WHERE col = ? AND id = ?', [this.name, item._id], doNothing, error);
              return cb();
            } else {
              return cb();
            }
  ...
  seed(doc, success, error) {
    ...
    return this.db.transaction(tx => {
      return tx.executeSql("SELECT * FROM docs WHERE col = ? AND id = ?", [this.name, doc._id], (tx, results) => {
        ...
        if (results.rows.length === 0) {
          return tx.executeSql("INSERT INTO docs (col, id, state, doc) VALUES (?, ?, ?, ?)", [this.name, doc._id, "cached", JSON.stringify(doc)], function() {
            if (success) { return success(doc); }
          }
          , error);
        } else {
    ...
    return this.db.transaction(tx => {
      return tx.executeSql("SELECT * FROM docs WHERE col = ? AND id = ?", [this.name, doc._id], (tx, results) => {
        ...
        if ((results.rows.length === 0) || (results.rows.item(0).state === "cached")) {
          ...
          // If _rev present, make sure that not overwritten by lower _rev
          if (!existing || !doc._rev || !existing._rev || (doc._rev >= existing._rev)) {
            return tx.executeSql("INSERT OR REPLACE INTO docs (col, id, state, doc) VALUES (?, ?, ?, ?)", [this.name, doc._id, "cached", JSON.stringify(doc)], function() {
              if (success) { return success(doc); }
            }
            , error);
          } else {
