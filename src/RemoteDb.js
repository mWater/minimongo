...
class Collection {
  ...
  find(selector, options = {}) {
    return { fetch: (success, error) => {
      ...
      } else if (this.usePostFind && (JSON.stringify({ selector, sort: options.sort, fields: options.fields }).length > 500)) {
        method = "post";
      } else {
        method = "get";
      }
  ...
  upsert(docs, bases, success, error) {
    ...
    if (items.length === 1) {
      ...
      // POST if no base, PATCH otherwise
      if (basesPresent) {
        return this.httpClient("PATCH", this.getUrl(), params, { doc: _.pluck(items, "doc"), base: _.pluck(items, "base") }, result => success(result)
        , function(err) {
          if (error) { return error(err); }
        });
      } else {
        return this.httpClient("POST", this.getUrl(), params, _.pluck(items, "doc"), result => success(result)
        , function(err) {
          if (error) { return error(err); }
        });
      }
