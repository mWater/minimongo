...
ALTER TABLE docs ADD COLUMN base TEXT;`, [], doNothing, ((tx, err) => error(err)));
  ...
  removeCollection(name, success, error) {
    ...
    return this.db.transaction(tx => tx.executeSql("DELETE FROM docs WHERE col = ?", [name], success, ((tx, err) => error(err)))
    , error);
  }
...
class Collection {
  ...
  upsert(docs, bases, success, error) {
    ...
    return this.db.transaction(tx => {
      ...
      // Get bases
      bases = {};
      return async.eachSeries(ids, (id, callback) => {
  ...
  resolveUpserts(upserts, success, error) {
    ...
    // Find records
    return this.db.transaction(tx => {
      return async.eachSeries(upserts, (upsert, cb) => {
  ...
  uncache(selector, success, error) {
    ...
    return this.db.transaction(tx => {
      return tx.executeSql("SELECT * FROM docs WHERE col = ? AND state = ?", [this.name, "cached"], (tx, results) => {
        ...
        // Add all non-local that are not upserted or removed
        return async.eachSeries(toRemove, (id, callback) => {
          // Only safely remove if removed state
          return tx.executeSql('DELETE FROM docs WHERE state="cached" AND col = ? AND id = ?', [this.name, id], () => callback()
          , ((tx, err) => error(err)));
        }
        , err => {
          if (err) {
